...
def parse_player_stats(url):
    ...
    for row in tbody.find_all("tr"):
        ...
        player = [rating, acs, kd, adr, kast, kpr, apr, fkdratio, clp]
        averages.append(player)
        print(averages)


    return [round(sum(col) / len(col), 2) for col in zip(*averages)]

if __name__ == "__main__":
    url = "https://www.vlr.gg/event/stats/2380/champions-tour-2025-emea-stage-1"
    stats = parse_player_stats(url)
    if stats:
        print(stats)
